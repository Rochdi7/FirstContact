<x-app-layout>
    ...
    <x-slot name="toolbar">
        <x-toolbar title="{{__('users.title')}}" :breadcrumb="Breadcrumbs::render('user')">
            <a href="{{ route('admin.users.create') }}" class="btn btn-flex btn-primary h-40px fs-7 fw-bold">{{__('users.create')}}</a>
        </x-toolbar>
    </x-slot>
        ...
        <script>
            $(function () {
                ...
                let table = $('#datatable_users').DataTable({
                    ...
                    columns: [
                        {data: 'avatar', name: 'first_name',className: 'text-start'},
                        {data: 'roles', name: 'roles', orderable: false,className: 'text-start'},
                        {data: 'last_activity_blade', name: 'last_activity',className: 'text-start'},
