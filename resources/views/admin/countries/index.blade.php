<x-app-layout>
    ...
    <x-slot name="toolbar">
        <x-toolbar title="{{__('countries.title')}}" :breadcrumb="Breadcrumbs::render('country')">
            <a href="{{ route('admin.countries.create') }}" class="btn btn-flex btn-primary h-40px fs-7 fw-bold">{{__('countries.create')}}</a>
        </x-toolbar>
    </x-slot>
    ...
    <div class="card">
        ...
        @include("admin.countries.form-search")

        <!--begin::Card body-->
        <div class="card-body">
           @include('admin.countries.datatable')
        </div>
        <!--end::Card body-->
    </div>
    <!--end::Card-->
    ...
    @push('scripts')
        ...
        <script>
            $(function () {
                ...
                let table = $('#datatable_countries').DataTable({
                    responsive: true,
                    processing: true,
                    serverSide: true,
                    language: {
